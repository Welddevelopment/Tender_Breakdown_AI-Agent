<p align="center">
  <a href="https://www.bidframe.org">
    <img src="frontend/public/brand/bidframe-logo-horizontal.svg" width="360" alt="Bidframe" />
  </a>
</p>

<p align="center">
  <a href="https://www.bidframe.org/demo"><img alt="Open the worked example" src="https://img.shields.io/badge/see%20it%20run-/demo-8A2D2A?style=for-the-badge" /></a>
...
<p align="center">
  <a href="https://www.bidframe.org/demo"><strong>See it run (guided demo)</strong></a>
  |
  <a href="https://www.bidframe.org">Live product</a>
  |
...
**See it run:** open the guided worked example at **[https://www.bidframe.org/demo](https://www.bidframe.org/demo)**. It is the real product surface, frozen on a pre-baked Bradwell grounds-maintenance tender, so it works without login, backend, or API key.

**Use it for real:** the live product is at **[https://www.bidframe.org](https://www.bidframe.org)**. Sign in with the demo accounts below to test the auth flow and the live two-account collaboration.

## Sign in and test it

Two shared demo accounts are set up so a reviewer can log in and see live collaboration on a real tender.

| Role | Email | Password |
|---|---|---|
| Alice (bid manager) | `alice@bidframe.co.uk` | `alicepw123` |
| Bob (compliance reviewer) | `bob@bidframe.co.uk` | `bobpw12345` |

Sign in as **Alice** at [https://www.bidframe.org](https://www.bidframe.org) and open the shared tender. Then sign in as **Bob** in a separate browser or profile to see live collaboration: when Alice shares a tender and Bob approves, edits, or flags a requirement, the decision lands on Alice's screen live, stamped "by Bob", with no refresh.

These are shared demo accounts for evaluation, so treat any data in them as public.
...
3. **Flags deal-breakers** so the bid team reviews bid-killers first. A two-stage engine (a deterministic net plus a model pass) never silently drops a gate.
...
The guided demo runs on a pre-baked Bradwell grounds-maintenance tender.
...
A tender is reviewed by a team: bid, compliance, and commercial. In the live product you can **share a tender with a colleague or a standing team** and work it together **in real time**. When one reviewer approves, edits, flags, or comments on a requirement, it appears on the others' screens **live, no refresh** (streamed over Server-Sent Events). Every decision is **attributed to who made it**, stamped server-side so it cannot be forged. A shared **activity feed** shows the team's actions as they happen, each row carries the initials of whoever signed it off, and any requirement can hold a **comment thread** for the back-and-forth. Reusable **teams** mean you set your bid group up once instead of re-inviting per tender. The audit trail *is* the collaboration.

Two people on one tender, live: [`ops/demo-bob-script.md`](ops/demo-bob-script.md) walks the two-account flow end to end.
...
Upload, to review, to draft, to export. You upload the tender pack, Bidframe reads it into a compliance matrix with deal-breakers first, you review each line and let it draft answers from your evidence, then you export the result you have checked.
...
|---|---|
| [`frontend/`](frontend) | Next.js 16, React 19, Tailwind. Compliance matrix, source panel, answer workspace, graph, demo, pitch. |
...
| `PATCH` | `/requirements/{id}` | Save an approve, edit, or flag decision, attributed server-side to the signed-in user. |
...
The frontend builds against the same shape as the backend returns. That is why the demo can use a frozen real tender run and the live product can swap to the API without changing the UI.
