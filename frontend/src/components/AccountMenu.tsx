"use client";

import Link from "next/link";
import { useRouter } from "next/navigation";
import { useAuth } from "@/context/AuthContext";

// A quiet account control for the header: who is signed in, their teams, and a way
// out. Text only, mono, muted — it never competes with the page's one primary action.
// Teams is account-level (a global collaboration destination, not a per-tender view),
// so it lives here rather than in the per-tender SectionNav. Renders nothing when there
// is no signed-in account (the mock showcase build), so the header is unchanged there.
export function AccountMenu() {
  const { user, status, signOut } = useAuth();
  const router = useRouter();

  if (status !== "authed" || !user) return null;

  return (
    <div className="flex items-center gap-2.5 text-xs">
      <Link
        href="/teams"
        className="rounded-sm font-mono text-ink-muted underline decoration-hairline decoration-1 underline-offset-4 transition-colors hover:text-ink focus:outline-none focus-visible:ring-2 focus-visible:ring-forest focus-visible:ring-offset-2 focus-visible:ring-offset-paper"
      >
        Teams
      </Link>
      <span aria-hidden className="text-hairline">
        ·
      </span>
      <span
        className="hidden max-w-[16ch] truncate font-mono text-ink-muted sm:inline"
        title={user.email}
      >
        {user.email}
      </span>
      <span aria-hidden className="hidden text-hairline sm:inline">
        ·
      </span>
      <button
        type="button"
        onClick={() => {
          signOut();
          router.push("/");
        }}
        className="rounded-sm text-ink-muted underline decoration-hairline decoration-1 underline-offset-4 transition-colors hover:text-ink focus:outline-none focus-visible:ring-2 focus-visible:ring-forest focus-visible:ring-offset-2 focus-visible:ring-offset-paper"
      >
        Sign out
      </button>
    </div>
  );
}
