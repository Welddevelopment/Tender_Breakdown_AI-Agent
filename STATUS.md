...
| **Generalist** | reconcile/dedupe · confidence routing · eval harness · answer-draft | 🟢 **reconcile + eval shipped** — `engine/` on main, 60 tests green; 6→5 dedupe (ISO dupe merges @0.9928), closed reconcile→eval loop scores recall/precision/gating 1.0 on the mock | Run reconcile on backend's real raw list (SPSO); hand-label SPSO into `engine/gold/` for a real headline number; confidence calibration + answer-draft (Day 3) | not blocked |
