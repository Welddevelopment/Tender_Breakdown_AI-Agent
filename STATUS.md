...
| **Frontend** | compliance matrix · source panel · decision controls · graph view · demo | 🟢 **wireframes live + deployed** — separate routes + persistent navbar, source+decision **slide-over drawer** (approve/edit/flag), **gating hero**, **React Flow** relationship graph (criteria_ref + depends_on), **upload** flow; in-memory decision state survives navigation; live on Vercel | Rebrand UI **Tender Breakdown → Bidframe**; mirror `answer`/`open_questions`/`capability_docs` into `types/requirement.ts` + a couple mocks (additive); answer + gap-interview UI (Day 3–4); design-system pass | backend mock `/requirements` for real-data swap |
