...
| **Frontend** | compliance matrix · source panel · decision controls · graph view · demo · landing | 🟢 **landing page overhauled to hi-fi (civic record)** — public landing at `/` (matrix now `/review`): centred two-line hero + supabase-style tilted **real-product** sheet (cursor-parallax, settles on load), two ink bands (proof ledger + closing), faint ledger grid + forest botanical framing, dimensional confidence beads, approval stamp; **worked-example entry** (clickable shot + CTA → `/review`) for cold traffic, **waitlist email capture**; left-aligned, slop-checked, one 2px rule. tsc + lint + `next build` green (F-013). Prior: answer/evidence panel, gap-interview (`/answers`), live API wiring (mock by default) | **Set `NEXT_PUBLIC_BOOKING_URL` + `NEXT_PUBLIC_WAITLIST_ENDPOINT` before outreach**; set `NEXT_PUBLIC_API_BASE_URL` → live Render URL in Vercel for hosted live data | nothing blocking |
