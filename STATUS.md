...
| **Generalist** | reconcile/dedupe · confidence routing · eval harness · answer-draft | 🟢 **engine LIVE + aggregate harness** — `engine/` on main (70 tests green); **wired into `backend/app/pipeline.py`** (conservative dedupe + noisy-OR + safety escalation + `needs_review`; OpenAI = 9% honest flag), import-safe fallback. SPSO eval **recall ~0.9 / gating recall 1.0 / 0 dangerous**; **aggregate `eval_all` across all gold** (eval-manifest); **66pp tender survives extract→reconcile, no crash** (35% robustness); **`needs_review` calibrated 0.75→0.70 vs SPSO gold (finding: LLM confidence weakly informative — G-007)**. G-005/6/7 (72 tests) | @all: finish handoff gold (museum); @j: render.yaml deploy + **demo must use OpenAI** (heuristic=0 gating recall); answer-draft (Day 3) | not blocked |
